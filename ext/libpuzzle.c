...
static VALUE new(VALUE klass, VALUE file_path){
  ...
  //StringValueCStrはstringがnulを含んでいるかチェックできる
...
static double _distance(VALUE p1, VALUE p2){
  ...
  // p1からPuzzle構造体を取得してポインタをpuzzle_1にセットする
...
void Init_puzzle_ext(void){
  ...
  //rb_cPuzzleオブジェクトに"uncompress"を定義しま�<M-Space>。
  //メソッドの実態をuncompressに関数ぽいんたを与え、引数のタイプをargcにわた�。
  //argc=1は、引数の数は1、16個以上は使えない
